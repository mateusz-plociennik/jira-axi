...
- `release list` works only on Jira Cloud: jira-cli calls the v3 versions API, which Jira Server/Data Center lacks, so there it fails with `NOT_FOUND`. Don't retry with other keys; tell the user.
